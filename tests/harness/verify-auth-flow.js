...
async function main() {
  ...
  try {
    ...
    console.log('\n-- 5 - password reset --');
    A('forgot-password (real email) → 200 (no enumeration)', (await H().post('/api/auth/forgot-password', { email: EMAIL })).status === 200);
    ...
    const tok = (await c.query(`SELECT pr.data->>'token' AS t FROM password_resets pr JOIN users u ON u.id=pr.user_id WHERE lower(u.data->>'email')=lower($1) ORDER BY pr.id DESC LIMIT 1`, [EMAIL])).rows[0]?.t;
    A('a reset token was stored', !!tok, `token=${tok}`);
    ...
    await c.query(`INSERT INTO password_resets (user_id, entity_id, data, created_at, updated_at) VALUES ($1,NULL,$2,NOW(),NOW())`,
      [uid, { token: 'EXPIRED-TOKEN', expires: new Date(Date.now() - 3600e3).toISOString() }]);
