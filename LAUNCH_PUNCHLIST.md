...
- [x] **Audit-log anomaly DETECTION — ✅ DONE 2026-09-14** — `audit-anomalies.js` + `GET /api/admin/audit-anomalies` detect accountant cross-client access, activity spikes, mass delete; `verify-audit-anomalies` 10/0. REMAINING: wire to a push/email alert channel; mass-export + impossible-travel need those events audited first.
