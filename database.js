...
async function seedUserData(userId) {
  // Guard: don't seed if user already has ANY entities (even empty ones)
  const existing = await pool.query(`SELECT id FROM entities WHERE user_id = $1 LIMIT 1`, [userId]);
