...
> **F141 (NEW, this finding's spillover — do NOT fold in):** (a) the Bills-page payables KPI is effectively **absent** at runtime — pages.js `renderBills` renders only the list, no payables card, so `#bills-total` shows its static default; (b) the dead stubs.js bills subsystem should be deleted (F75/F136 class); (c) **AR mirror still open** — `app-main.js:2330` and `finflow-api-wiring-extra.js:1082` compute *invoice* outstanding as `Σ amount over status!=='paid'`, ignoring `amount_paid`, so a partially-paid INVOICE overstates AR on those two surfaces (the F56 class on extra surfaces). Each is code-read, unexecuted.
