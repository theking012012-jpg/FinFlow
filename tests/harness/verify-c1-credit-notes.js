'use strict';
/**
 * verify-c1-credit-notes.js — PROVE (Rule 14) that the partial UNIQUE index on the credit_notes
 * idempotency token + the 23505 handler make POST /api/credit-notes safe against the concurrent /
 * slow double-submit the 5s findRecentDuplicate pre-check cannot stop (C1 class, Wave 1).
 *
 * credit_notes is user-scoped (no entity_id stored), so the token is the key and the existing null
 * entityId pre-check is already consistent (no scope gap, unlike payments_received). Two legitimate
 * credit notes to the same customer for the same amount are allowed, so token, not UNIQUE(customer,
 * amount).
 *
 *   node -r ./tests/harness/clock.js tests/harness/verify-c1-credit-notes.js
 *   NO_INDEX=1 node -r ./tests/harness/clock.js tests/harness/verify-c1-credit-notes.js   (CONTROL)
 */
...
const LOGIN = { email: 'cn-c1@finflow.test', password: 'harness-password-not-a-secret' };
...
(async () => {
  ...
  try {
    ...
    const cnCount = async (uid, customer) =>
      Number((await c.query(
        `SELECT COUNT(*) n FROM credit_notes WHERE user_id=$1 AND data->>'customer'=$2`, [uid, customer]
      )).rows[0].n);
    const ageOut = (uid, customer) =>
      c.query(`UPDATE credit_notes SET created_at = NOW() - INTERVAL '30 seconds' WHERE user_id=$1 AND data->>'customer'=$2`,
        [uid, customer]);

    server = await bootServer(scratch.url);      // runs initDB → creates idx_credit_notes_idem_key
    console.log(`\n  MODE: ${WITH_INDEX ? 'WITH INDEX (the fix)' : 'NO INDEX (Rule-14 CONTROL — proves the JS guard alone is insufficient)'}\n`);
    if (!WITH_INDEX) {
      const d = await c.query(`DROP INDEX IF EXISTS idx_credit_notes_idem_key`);
      console.log(`  (control) dropped idx_credit_notes_idem_key — command tag: ${d.command || 'DROP'}\n`);
    } else {
      const ix = await c.query(`SELECT indexdef FROM pg_indexes WHERE indexname='idx_credit_notes_idem_key'`);
      A('migration present: idx_credit_notes_idem_key exists (partial, on data->>idempotency_key)',
    ...
    const uid = (await c.query(
      ...
      [{ email: LOGIN.email, name: 'CN C1 Owner', plan: 'trial', role: 'owner',
    ...
    const eid = (await c.query(
      ...
      [uid, { name: 'CN C1 Co', currency: 'USD', is_active: 1 }]
    )).rows[0].id;
    ...
    const post = (body) => http.post('/api/credit-notes', body);
    ...
    const nA = await cnCount(uid, cA);
    if (WITH_INDEX) {
      A('A. concurrent same-token: exactly ONE credit-note row', nA === 1, `rows=${nA}  statuses ${r1.status}/${r2.status}`);
    ...
    const insRaw = () => c.query(`INSERT INTO credit_notes (user_id, entity_id, data) VALUES ($1,$2,$3)`, [uid, eid, rawData]);
    await insRaw();
    let rawRejected = false;
    try { await insRaw(); } catch (e) { rawRejected = (e.code === '23505'); }
    const nRaw = await cnCount(uid, 'raw-C');
    if (WITH_INDEX) {
    ...
    // ── D. different tokens, same customer+amount, spaced >5s → TWO rows (two credit notes preserved) ──
    const cD = 'legit-D';
    ...
    const nD = await cnCount(uid, cD);
    A('D. different tokens, same customer+amount: TWO rows (two legitimate credit notes preserved — token, not UNIQUE(customer,amount))',
    ...
    const nD2 = await cnCount(uid, cD2);
    ...
    // ── E2. no token, same customer+amount, <5s apart → ONE row (pre-check consistent: user-scoped) ──
    const cE2 = 'nokey-E2';
    const e2a = await post({ customer: cE2, amount: 700 });
    const e2b = await post({ customer: cE2, amount: 700 });
    const nE2 = await cnCount(uid, cE2);
    A('E2. no-token, same customer+amount, <5s apart: ONE row (pre-check still active for token-less requests — user-scoped, null entityId consistent)',
