...
- [x] **Audit-log anomaly DETECTION — ✅ DONE 2026-09-14** — `audit-anomalies.js` + `GET /api/admin/audit-anomalies` detect accountant cross-client access, activity spikes, mass delete; `verify-audit-anomalies` 10/0. DELIVERY ✅ 2026-09-14 — email digest via Resend (`POST /api/admin/audit-anomalies/notify`, `SECURITY_ALERT_EMAIL`). REMAINING: mass-export + impossible-travel need those events audited first; optionally schedule the scan.
