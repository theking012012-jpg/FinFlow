'use strict';
/**
 * verify-c1-sales-receipts.js — PROVE (Rule 14) that the partial UNIQUE index on the sales_receipts
 * idempotency token + the 23505 handler make POST /api/sales-receipts safe against the concurrent /
 * slow double-submit the 5s findRecentDuplicate pre-check cannot stop (C1 class, Wave 1).
 *
 * sales_receipts STORES entity_id, and its token-less pre-check previously hardcoded entityId=null —
 * the same entity-scope bug fixed in payments_received. Case E2 guards that fix. Two legitimate
 * receipts to the same customer for the same amount are allowed, so token, not UNIQUE(customer,amount).
 *
 *   node -r ./tests/harness/clock.js tests/harness/verify-c1-sales-receipts.js
 *   NO_INDEX=1 node -r ./tests/harness/clock.js tests/harness/verify-c1-sales-receipts.js   (CONTROL)
 */
...
const LOGIN = { email: 'sr-c1@finflow.test', password: 'harness-password-not-a-secret' };
...
(async () => {
  ...
  try {
    ...
    const srCount = async (uid, customer) =>
      Number((await c.query(
        `SELECT COUNT(*) n FROM sales_receipts WHERE user_id=$1 AND data->>'customer'=$2`, [uid, customer]
      )).rows[0].n);
    const ageOut = (uid, customer) =>
      c.query(`UPDATE sales_receipts SET created_at = NOW() - INTERVAL '30 seconds' WHERE user_id=$1 AND data->>'customer'=$2`,
        [uid, customer]);

    server = await bootServer(scratch.url);      // runs initDB → creates idx_sales_receipts_idem_key
    console.log(`\n  MODE: ${WITH_INDEX ? 'WITH INDEX (the fix)' : 'NO INDEX (Rule-14 CONTROL — proves the JS guard alone is insufficient)'}\n`);
    if (!WITH_INDEX) {
      const d = await c.query(`DROP INDEX IF EXISTS idx_sales_receipts_idem_key`);
      console.log(`  (control) dropped idx_sales_receipts_idem_key — command tag: ${d.command || 'DROP'}\n`);
    } else {
      const ix = await c.query(`SELECT indexdef FROM pg_indexes WHERE indexname='idx_sales_receipts_idem_key'`);
      A('migration present: idx_sales_receipts_idem_key exists (partial, on data->>idempotency_key)',
    ...
    const uid = (await c.query(
      ...
      [{ email: LOGIN.email, name: 'SR C1 Owner', plan: 'trial', role: 'owner',
    ...
    const eid = (await c.query(
      ...
      [uid, { name: 'SR C1 Co', currency: 'USD', is_active: 1 }]
    )).rows[0].id;
    ...
    const post = (body) => http.post('/api/sales-receipts', body);
    ...
    const insRaw = () => c.query(`INSERT INTO sales_receipts (user_id, entity_id, data) VALUES ($1,$2,$3)`, [uid, eid, rawData]);
    await insRaw();
    let rawRejected = false;
    try { await insRaw(); } catch (e) { rawRejected = (e.code === '23505'); }
    const nRaw = await srCount(uid, 'raw-C');
    if (WITH_INDEX) {
    ...
    const nD2 = await srCount(uid, cD2);
    ...
    // ── E2. no token, same customer+amount, <5s apart → ONE row (GUARDS the entity-scope pre-check fix) ──
    const cE2 = 'nokey-E2';
    const e2a = await post({ customer: cE2, amount: 700 });
    const e2b = await post({ customer: cE2, amount: 700 });
    const nE2 = await srCount(uid, cE2);
    A('E2. no-token, same customer+amount, <5s apart: ONE row (pre-check entity-scope fix — was hardcoded null vs entity_id insert)',
