...
- [x] **Audit-log anomaly DETECTION — ✅ DONE 2026-09-14** — `audit-anomalies.js` + `GET /api/admin/audit-anomalies` detect accountant cross-client access, activity spikes, mass delete; `verify-audit-anomalies` 10/0. DELIVERY ✅ + SIGNALS EXPANDED ✅ 2026-09-14 — email digest via Resend (`POST /api/admin/audit-anomalies/notify`, `SECURITY_ALERT_EMAIL`); login + document-download events audited → `login_multi_ip` + `mass_export` signals added (`verify-anomaly-events` green). Plus `failed_login_burst` ✅ 2026-09-14 — failed logins audited (owner + accountant, with IP) → brute-force detection (`verify-anomaly-failed-login` green). NOTE: CSV/PDF report exports are client-side, not server-auditable. AUTO-SCAN ✅ 2026-09-14 — periodic scan→email on boot (`startAnomalyMonitor`, guarded by `SECURITY_ALERT_EMAIL`; `verify-anomaly-monitor` green). Optional: true geo-velocity impossible-travel (needs a geo service).
- [~] **Error monitoring (Sentry) + uptime alerting** — UPTIME ✅ 2026-09-14: `GET /healthz` (unauth, DB check, 200/503; `verify-healthz` green) for external monitors. REMAINING: Sentry (needs your DSN).
...
- [x] **Connector-token encryption coverage — ✅ HARNESS-PROVEN 2026-09-14** — `verify-connector-token-encryption` 11/0 reads the stored blob and asserts ciphertext + plaintext-never-present (WiPay api_key, Woo consumer key/secret); audit confirmed all 13 connectors encTok stored credentials (Codat/Belvo store no per-user secret). No gaps.
- [x] **KYC Phase C — per-body registry-portal links — ✅ DONE 2026-09-14.** `kyc-registry.js` maps a professional body to its OFFICIAL verified register (ACCA, ICAEW, ICAS, CIMA/CGMA, US CPA/NASBA CPAverify, CPA Ontario, CPA Alberta; URLs verified 2026-09-14), wired into the admin accountant modal; unmapped bodies (e.g. ICATT) fall back to the scoped web search. `verify-kyc-registry` 14/0.
...
1. **FX base-currency consolidation** — the #1 credibility item. **SERVER LAYER ✅ SHIPPED 2026-09-14 (`c005cbd`):** `computeBooks(entityId=null)` now converts every leg per-entity to the account base currency (`verify-fx-consolidation` 12/0; 65/65 money sweep; single-currency byte-identical). **REMAINING (client layer):** the main dashboard consolidated is summed client-side by `getConsolTotal`/`fxConvert` (index.html), which does NOT raw-sum — it RE-CONVERTS each entity via STATIC spot rates, so it DIVERGES from the server (Rule 2) and DOUBLE-CONVERTS when a display currency is active. Fix: add `?entity_id=all` → entityId=null, point the client at it, and REMOVE the client fxConvert. Full detail: `FX_CONSOLIDATION_DESIGN.md` STATUS block.
