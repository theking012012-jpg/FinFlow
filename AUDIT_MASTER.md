...
| F2 | 🔴 Critical | Partial `PUT` nulls `amount` and deletes fields on recurring bills/invoices (whole-object patch with `undefined` keys); reproduced corrupt JSONB | `server.js:1859,1931`; `database.js:674` | Build patches conditionally (`if (x!=null) patch.x=…`) as invoices/expenses already do | agent |
...
| F4 | 🟠 High | Global error handler sits **before ~42 routes** → those return HTML/stack instead of JSON (stack leak in non-prod) | `server.js:2521` (routes 2617–3560) | Move error handler + `/api` & `*` 404 fallbacks to the very bottom | agent |
...
2. **F2** — silent data corruption on every partial edit.
3. **F4** — cheap; unblocks correct JSON errors for 42 endpoints (do before/with F14/F15).
