...
(async () => {
  ...
  try {
    ...
    // F150 seed-debt (test-only): this harness INTENTIONALLY seeds a legacy account-wide receipt
    // (R0, entity_id = NULL) that must stay visible to BOTH entities. chk_*_entity_nn is NOT VALID
    // (enforces new writes, tolerates pre-existing NULLs), so drop it at seed time — the product
    // still stamps entity_id on real writes (verify-f150c, 33/0).
