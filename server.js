...
// Records a page view per unique visitor per UTC day for the admin Traffic panel,
// with coarse IP-derived location and time. Registered HERE — before the "/" and
// "/app" routes and before express.static — so it sees every real page navigation.
//
// Non-blocking by construction: the work is deferred to the response 'finish' event
// and fired without await, and every geo/DB error is swallowed, so tracking can never
// slow down or break a page load. Only genuine navigations are counted — GET only,
// nothing under /api, nothing with a file extension (static assets), and only
// successful (<400) responses.
//
// fast-geoip is an OFFLINE ip→location database (no external calls, ~1MB RAM). It is
// lazy-required and wrapped: if it is ever missing the app still records views with a
// null location instead of throwing.
let _geoipMod = null, _geoipTried = false;
function _getGeoip() {
  if (!_geoipTried) { _geoipTried = true; try { _geoipMod = require('fast-geoip'); } catch (_) { _geoipMod = null; } }
  return _geoipMod;
}
async function _lookupGeo(ip) {
  try {
    ...
    // Skip loopback / private / link-local / ULA — no meaningful public location.
    if (/^(localhost|127\.|10\.|192\.168\.|169\.254\.|172\.(1[6-9]|2\d|3[01])\.|::1$|fc|fd|fe80)/i.test(clean)) return null;
    const r = await g.lookup(clean);
    if (!r) return null;
    return { country: r.country || null, region: r.region || null, city: r.city || null };
  } catch (_) { return null; }
}
async function _recordPageView(v) {
  const geo = await _lookupGeo(v.ip);
  await pool.query(
    `INSERT INTO page_views
       (visitor_key, view_day, ip_address, country, region, city, path, referrer, user_agent, user_id, views, first_seen, last_seen)
     VALUES ($1, (NOW() AT TIME ZONE 'UTC')::date, $2,$3,$4,$5,$6,$7,$8,$9, 1, NOW(), NOW())
     ON CONFLICT (visitor_key, view_day) DO UPDATE SET
       views     = page_views.views + 1,
       last_seen = NOW(),
       country   = COALESCE(page_views.country, EXCLUDED.country),
       region    = COALESCE(page_views.region,  EXCLUDED.region),
       city      = COALESCE(page_views.city,    EXCLUDED.city)`,
  ...
  if (req.method === 'GET' && !_TRACK_SKIP_RE.test(req.path) && !/\.[a-z0-9]{1,8}$/i.test(req.path)) {
    res.on('finish', () => {
      try {
        ...
        const vkey = crypto.createHash('sha256').update((ip || '') + '|' + ua).digest('hex').slice(0, 40);
        _recordPageView({ vkey, ip, ua, ref, pth, uid }).catch(() => {});
      } catch (_) { /* tracking must never affect the response */ }
    });
  }
  next();
});
