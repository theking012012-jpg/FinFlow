...
- **Remaining:** UPDATE/DELETE on the money tables (old→new snapshot); business-record CRUD (`customers`, `vendors`, `inventory`/`inventory_movements`, `chart_of_accounts`, `entities`); and the lower-stakes accountant handlers (notes/flag/checklist/message/notify/bill-client). Pure UI/settings toggles are out of scope.
