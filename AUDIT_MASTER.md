...
- **F137-j/k/l/m (Tax reports)** — built this cycle, honestly from real data with clear limitation labels (no invented tax logic): **Tax-Deductible Expenses** (deductible expenses by category, Σ == total, real `deductible` flag); **Income Tax Estimate** (revenue − deductibles from `/api/tax-filing`, × an **editable, persisted tax rate** ÷ 4, behind a "rough estimate — not tax advice" banner). The rate defaults to 25% but is a live editable field that recomputes on input and saves to `user_settings.data.tax_rate` via `PUT /api/settings` (a new server allowlist field). **This also closes an orphan (call it F138):** the accountant portal's Tax Summary already read `settings.data.tax_rate` (`accountant-routes.js:547`) and warned "Client has not set a tax rate", but the main app had NO input to set it — now it does, and one number drives both surfaces. (Separate minor finding: the accountant payroll CSV still reads `p.tax_rate`, a field F8 deleted → exports 0; log for cleanup.) **VAT Return** (honest "VAT/GST is not tracked" state — FinFlow records no tax on transactions, F123 class, never fabricated); **1099/W-2 Summary** (per-employee W-2 wages from payroll runs, gross = Σ line items basis C, with an explicit "contractor/1099 not tracked" note). Verified 14/14 jsdom fail-then-pass. **All 12 reports now render their own content.**
