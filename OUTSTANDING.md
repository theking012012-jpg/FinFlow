...
- **Mobile performance (Lighthouse 54)** — ✅ **DONE 2026-09-24.** Compression was already on; the remaining lever, the **minify build**, shipped: `scripts/minify.js` (terser) writes `public/.min/<name>`, and server.js transparently serves the minified copy under the SAME URL when fresh (mtime guard), keeping `no-store` and leaving index.html + the SW manifest + the bundle drift-guard untouched. Purely additive + fail-safe (no terser / minify error / stale artifact → serves the readable original). ~43% smaller JS (905 KB → 517 KB). `prestart` runs it after `bundle.js`; `public/.min/` is gitignored. Verified by `verify-min-serving.js` (8/0).
...
- **Invoicing + pricing overhaul — ✅ DONE 2026-09-12.** Final decision: **Pro invoicing is UNLIMITED**, backed by a **500/month fair-use / anti-abuse ceiling** (env-tunable `PRO_INVOICE_CEILING`; counted by created_at, un-gameable; sits after an idempotency pre-check so retries return the original row; Business/Scale truly uncapped). Copy flipped to "Unlimited invoicing & quotes"; the `invoice_limit` upsell reworded to the fair-use framing. `verify-invoice-cap` 8/0 (retuned to 500).
  ...
  - **Marketing landing (`public/landing.html`) synced 2026-09-12:** the standalone marketing pricing page (separate file from the in-app `page-pricing`) was updated to match — Business $249, new Scale card ($400/10 entities), "Unlimited invoicing & quotes" copy, comparison header $249, and the annual toggle to 20% ("Save 20%"). ✅ RESOLVED 2026-09-24: trial length is **30 days** (backend already sets `trial_ends` to +30d in both signup paths). The only stray copy was the accountant-invite email in `accountant-routes.js` ("14-day trial"), now "30-day". Landing + in-app + FAQ all say 30 days.
