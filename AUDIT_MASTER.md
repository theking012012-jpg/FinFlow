...
### F196 🟡 MEDIUM — Document letterhead shows the ACCOUNT business, not the issuing ENTITY: a per-user setting applied to per-entity output — **NEW (2026-08-27, execution-verified) → 🟠 TIER 1 FIXED (committed); TIER 2 (per-entity profile) OPEN**
**Status:** 🟠 **PARTIAL — Tier 1 (the name) shipped and execution-verified; Tier 2 (the rest of the letterhead) still OPEN.** Every document FinFlow renders is issued BY an entity, but the letterhead is built from an account-wide blob.
**Sighting:** with **Saige Holdings** the active entity, the invoice document letterhead reads **"Acme"** (the account's `business_name`).
**Root cause (READ-verified):** the whole letterhead is read from the **account-wide** settings blob. `GET /api/settings` (`server.js:1918`) is `SELECT * FROM user_settings WHERE user_id = $1 AND data->>'key' IS NULL` — **ONE row per account, no entity scoping** — holding `business_name / address / email / phone / tax_id / website`. It is loaded once at boot (`finflow-api-wiring.js:51` `loadSettingsFromDB`) into the `s-*` inputs and **never reloads on entity switch**. Entities themselves carry only `name, currency, color, timezone, country` (`server.js:1076`) — no letterhead contact fields at all. So every entity's documents show the same account letterhead.
**Class (Rule 13) — this is the Rule 10 "UNDER INVESTIGATION" shape: a setting stored PER-USER but applied to PER-ENTITY output** (the same shape as timezone / fiscal-year / display-currency). It spans the ENTIRE letterhead — name, address, email, phone, tax-id, website, logo — on **every** document kind the viewer renders (`finflow-docview.js letterhead()`: invoice, bill, receipt, credit-note, quote, payment, vendor-credit), **plus** `buildInvoiceHTML` (`app-main.js:5958`, the Templates preview), which reads `s-biz-name` independently. Not just the name, and not just invoices.
**Second-order finding (surfaced while enumerating, NOT yet fixed):** the letterhead's `email`/`phone` are read from `s-email` / `s-phone`, which in the Settings page live under **"Your profile"** (`index.html:3490-3491`) — the USER's contact details — not under "Business profile". So the business letterhead currently prints the user's personal email/phone. Tier 2 resolves this by giving the entity its own `email`/`phone`; until then it stands as a known conflation.
**Tier 1 — FIXED (this commit):** the active entity's name now wins over the account blob, in BOTH letterhead writers:
- `finflow-docview.js letterhead()` → `name: (e && e.name) || val('s-biz-name') || 'Your Business'`
- `app-main.js buildInvoiceHTML` → `const bizName = (_activeEnt && _activeEnt.name) || …`
`(window.ENTITIES||[]).find(x => x.active)` is confirmed the active-entity accessor (the flag is `active`; `index.html:6182/6489`). This is coherent with the existing F149-b behaviour, where saving Settings already renames the ACTIVE entity (`rename_active_entity:true`, `server.js:1972`) — so the entity name is already the per-entity business name.
**Verified (execution, Rule 14):** `tests/harness/verify-docview-invoice.js` — **11/11 GREEN**. The letterhead assertion is now DISCRIMINATING: it sets `s-biz-name = 'Account Settings Biz Co'` and asserts the rendered document contains the **active entity's** name AND that the settings name **does not leak** — so the pre-fix code fails it. Full sweep **154/154 GREEN, 0 RED**.
**Tier 2 — OPEN (owner decision MADE 2026-08-27: FULL per-entity Business Profile).** Each entity gets its own letterhead profile (`business_name/address/email/phone/tax_id/website/logo` in the entity `data` JSONB, no migration), validated in `POST/PUT /api/entities`; `loadSettingsFromDB` loads the ACTIVE entity's profile and reloads on `switchEntity`; saving the Business Profile writes to the active entity; `letterhead()` consumes the entity profile with the account blob as fallback. The one-time copy of the existing account-wide profile onto the first/active entity is **owner-gated and its own commit (Rule 8)** — enumerate and report the rows first, never silently backfill. Until Tier 2 ships, address/email/phone/tax-id/website/logo on every document are still the ACCOUNT's, so a multi-entity account still shows one entity's contact details on another's documents. Full spec: `F194_MONEY_VIEWER_HANDOFF.md`.
**Files (Tier 1):** `public/finflow-docview.js` (`letterhead()`), `public/app-main.js` (`buildInvoiceHTML` bizName only), `tests/harness/verify-docview-invoice.js`. **Done when:** Tier 2 ships verified and the data move is owner-approved.

---

### F195 🟡 MEDIUM — Calendar-date DISPLAY labels shift a day west of UTC: `new Date(dateOnly).toLocaleDateString()` renders UTC midnight in the viewer's zone — **NEW (2026-08-27, execution-verified) → ✅ FIXED (committed)**
**Status:** ✅ **FIXED.** Display-only (no money figure moves), but it makes the SAME record read as two different calendar days on two surfaces of the same app, and the wrong day west of UTC for every viewer.
**Sighting:** an invoice with `due_date = '2026-08-16'` shows **"Aug 15"** in the Invoices list row but **"16 Aug 2026"** in the document viewer. The document is CORRECT.
**Root cause (READ-verified, then execution-confirmed):** the list mappers build the label as `new Date(r.due_date).toLocaleDateString('en-US', …)`. `new Date('2026-08-16')` parses a **date-only** string as **UTC midnight**; `.toLocaleDateString()` then renders that instant in the VIEWER's zone, so at GMT-4 it rolls back to Aug 15. This is the F87 / Rule 10 class applied to **display formatting** rather than period comparison — an accounting date is a calendar date, and converting it to an instant makes the rendered day depend on WHO IS ASKING.
**Class enumerated (Rule 13) — every date-only field formatted via `new Date(x).toLocaleDateString()` without the `+ ' 00:00'` local-midnight trick:**
- `finflow-api-wiring-postgres.js:160` (invoice due — the runtime winner), `:178` (expense date)
- `finflow-api-wiring-medium.js:85` (invoice due), `:125` (saveInvoice optimistic `dueStr`), `:280` (expense date)
- `app-main.js:1515` (invoice due), `:1524` (expense date), `:5489` (banking tx date)
- `finflow-api-wiring-final5.js:14` (`fmtDate()` — generic helper, date-only callers)
- **NOT affected (checked, left alone):** `app-main.js:5729/5757` use `+ ' 00:00'` → local midnight, no shift; `:5738` is `created_at`, a GENUINE TIMESTAMP (per Rule 10 that resolves against the entity tz — not a display-shift bug); `app-main.js:2580` is the DEAD `saveInvoice` copy (Rule 1 — the wiring override wins), left untouched rather than edited dead.
**Root fix (one formatter, not per-surface patches):** new `FinFlowDates.fmtLabel(v, opts)` in `public/finflow-dates.js` — reduces via the existing `_toYmd` (string slice: **no `Date`, no timezone**) and builds the label from the YMD parts, exactly as the document viewer's `dlabel` already did. Every date-only `toLocaleDateString` call site above now routes through it. Each call site keeps a `window.FinFlowDates ? … : raw` guard so a load-order miss degrades to the raw ISO string rather than throwing.
**Verified (execution, Rule 14 + the Rule 10 testing corollary):** `tests/harness/verify-date-label-tz.js` — **12/12 GREEN**. The matrix spans the **UTC sign boundary**: it asserts the OLD path shifts a WESTERN viewer to Aug 15 (reproduces the reported bug) and does NOT shift an EASTERN viewer (so a western-only matrix would have gone green on the bug), then that `fmtLabel` is TZ-invariant across both. Also covers the 1-Jan previous-year case, `…Z` timestamps, and the year/long-month options. The final assertion boots the real app in jsdom and reads the **rendered** invoice-list `due` value (executed value, not source text — Rule 5).
**Files:** `public/finflow-dates.js` (new `fmtLabel`), `public/finflow-api-wiring-postgres.js`, `public/finflow-api-wiring-medium.js`, `public/finflow-api-wiring-final5.js`, `public/app-main.js` (3 mappers), `tests/harness/verify-date-label-tz.js` (new). **Full sweep 154/154 GREEN, 0 RED.**

---

### F193 🟠 HIGH — Entity switch desync: an uncaught `chart.update()` throw aborted the switch mid-flight, so the sidebar and the in-page entity picker showed different entities — **NEW (2026-08-27, execution-verified) → ✅ FIXED & COMMITTED (`106cd8d`), confirmed live by owner**
**Status:** ✅ **FIXED, pushed `106cd8d`, owner-confirmed on prod.** Reported from prod: sidebar showed one entity (e.g. "Saige Holdings/USD") while the Scheduled Documents ENTITY dropdown showed another (e.g. "Acme") — they would not move together. Took three layers to reach the root; the first two were real but masked by the third.
**Root cause (the one that mattered) — an unguarded chart update aborts the switch:** the sidebar switcher runs `switchBusiness(id)` (`app-main.js:449`), which calls `setCurrency` (`:458`) **before** `switchEntity` (`:462`). Chain: `setCurrency` → `_applyDisplayCurrency` (`:388`) → `refreshAllPeriodData` (`:2276`) → `updateCharts` (`:2291`) → `charts.overview.update()` (`:5382`) / `charts.cash.update()` (`:5400`), **both unguarded**. On any page where the dashboard canvas is detached (user on Scheduled Documents etc.), Chart.js throws inside `update()` (`Cannot read properties of null (reading 'addEventListener')`, observed in the owner's console). That throw is **uncaught and synchronous**, so it aborts `switchBusiness` at `:458` — `switchEntity` (`:462`) never runs. The sidebar label was already set at `:455` (before the throw) → sidebar changes, entity/dropdown do not. `updateCharts` guarded only that the chart *instance* exists (`:5365`), not that its canvas is attached; its siblings in `_applyConvertedChart` (`:5082`/`:5101`) already wrap `.update()` in try/catch — these two were the only unguarded ones.
**Fix (`app-main.js:5382`, `:5400`):** wrap both `.update()` calls in try/catch, matching the `:5082`/`:5101` convention. `try{ charts.*.update(); }catch(_){}` — a detached-canvas throw is swallowed (the chart rebuilds when its page is next shown), so `updateCharts` can no longer abort its synchronous callers. Fixes the CLASS (Rule 13): every caller of `updateCharts`/`refreshAllPeriodData` (period changes, currency changes, entity switches) was exposed, not just `switchBusiness`.
**Two earlier layers, both correct but invisible until this fix (kept):**
- **`2e58582`** — `switchEntity` never reset `activeCurrency`, so `S()` stamped the previous entity's symbol on the new entity's native figures (Saige/USD rendered as TT$) until a refresh. Reset it, mirroring boot (`:1320`). Also re-renders the F94 tab on a switch.
- **`be892ba`** — `switchEntity` updated `ENTITIES` + the sidebar DOM text but not the parallel `businesses` model the sidebar switcher reads (`renderBusinessSwitcher`), so a direct switch repainted the old business. Mirror the active flag into `businesses` by `_dbId` + re-render the switcher.
**Verified (execution, Rule 14):** `verify-entity-switch-currency.js` injects a throwing chart into the real (lexical) `charts` object via `window.eval` and asserts `updateCharts` does not propagate the throw — **RED with the guard reverted, GREEN with it** (both executed). Plus the switch-follows assertions (sidebar ↔ dropdown, USD↔TTD round-trip). Full sweep **150/151**; the single red is `verify-c2-confirm-modal`, a pre-existing async-teardown flake (green 3/3 standalone; its unhandled rejection is in `renderEntities`, not charts) — **not this change**.
**Files:** `public/app-main.js:5382,5400` (guards), `tests/harness/verify-entity-switch-currency.js` (chart-throw regression). Earlier layers: `2e58582`, `be892ba`. **Done: committed `106cd8d`, confirmed live.**

---

### F192 🟡 MEDIUM — Bank-linking is region-incomplete and the "Banking" tab is a dead placeholder disconnected from the built backend — **NEW (2026-08-27, READ-verified)**
**Status:** 🔎 **OPEN — reported, nothing changed.** Not a money defect (connectivity/UX + coverage gap; imported rows land in the existing `source:'banking'` store, `server.js:4023`). READ-verified only — no execution.
**One-line:** two real bank aggregators are built (Plaid + Belvo), but the UI leads with Plaid for everyone, the region-appropriate provider is never offered, ~30 of the ~53 supported countries have NO aggregator at all, and the sidebar "Banking" tab shows a static "Coming Soon" card wired to none of it.
**Class enumerated (Rule 13 — both directions):**
1. **The "Banking" tab is a static placeholder.** `page-banking` (`index.html:2048-2060`) is hardcoded HTML — no `plaidConfigured` check, no JS. The "Banking" nav-item (`index.html:1149`) → `showPage('banking')` lands here, and it shows "Coming Soon" **even when Plaid/Belvo env vars are set**. Setting the keys changes nothing on this tab.
2. **The real linked-banks UI is orphaned.** `page-banking-biz` (`index.html:2929`, with `#plaid-linked-strip` fed from `/api/plaid/items`) is referenced only by its own `<div>` — **no `showPage('banking-biz')` exists anywhere.** Unreachable by navigation.
3. **The backend is genuinely built (so "Coming Soon" under-sells).** Plaid: `plaidConfigured` (`server.js:4605`), `/api/plaid/link-token` (`:4683`), `/exchange` (`:4702`), `/sync` (`:4745`), `/items` (`:4677`), AES-256-GCM token storage (`:4641`). Belvo (LatAm, env-gated): `belvoConfigured` (`server.js:5270`), `/api/belvo/status` (`:5287`), `/widget-token` (`:5293`) — its own comment: *"Plaid does NOT cover LatAm/Caribbean"* (`:5265`). The working link flow `ffLinkBank` (`index.html:4282`) IS reachable — but only from the **Connections** page's Plaid tile (`index.html:2583`), not the Banking tab.
4. **No region→provider routing.** The UI leads with Plaid regardless of the entity's country. Belvo is a separate tile the user must know to pick; the per-entity `country` (now writable via **F191**) is not used to choose the provider. `PLAID_COUNTRY_CODES` defaults to `US` only (`server.js:4604`).
5. **Coverage gap — the substantive one.** Bank-account aggregation exists only for Plaid regions (US/CA/UK/EU, when configured) + Belvo's core (MX/BR/CO). **~30 of the ~53 supported countries have no aggregator wired** — the entire Caribbean (incl. TT), Central America, and much of South America. WiPay covers the Caribbean but is **payments only** (`index.html:4227`: TT/JM/BB/GY/LC), not transaction/balance aggregation. These regions currently have no path to get a bank feed in.
6. **Card copy is wrong in two directions.** Over-promises *"Live balance sync every 15 minutes"* (`index.html:2056`) — **no such scheduler exists**; sync is on-demand `POST /api/plaid/sync` only (`server.js:4745`), and the only `15*60*1000` in `server.js` are auth rate-limiters. Under-sells with *"Coming Soon"* for a built feature. And *"powered by Plaid"* names only Plaid, misleading for non-Plaid regions.
**⚠️ OWNER DECISION (gates the fix, OPEN):** for the ~30 uncovered countries, is **manual statement import** (CSV/OFX/QFX into the existing `source:'banking'` store — every region ships, method varies by coverage) the launch answer, or should a regional aggregator be sourced first (delays those regions)? This decides the scope of the remediation.
**Intended design (once decided):** route by the entity's `country` — Plaid where covered, Belvo where covered, manual import otherwise — and show the "Coming Soon" card only when `!plaidConfigured() && !belvoConfigured()`. Wire the Banking tab to `page-banking-biz`/`ffLinkBank`. Fix the copy (drop the false 15-min claim; name the region-appropriate provider). Tracked in `OUTSTANDING.md`.
**Files:** `public/index.html` (2048-2060 Banking tab, 2929 orphaned page, 1149 nav, 2583 Connections wiring, 4227 WiPay, 4282 ffLinkBank), `server.js` (4594+ Plaid, 5269+ Belvo). **Done when:** the owner decision is made and the routing + fallback + tab wiring + copy ship verified — or the scope is explicitly documented as deferred.
