...
- **Bank region-strict routing — ✅ DONE 2026-09-12.** `ffBankLinkFromPage` now routes a country ONLY to its own region's aggregator (BELVO_MARKETS→Belvo, PLAID_MARKETS[US/CA/UK+EU]→Plaid); no cross-region fallback in either direction; uncovered (TT/Caribbean) → manual OFX/CSV. `verify-banking-region-routing` 15/0. (Provider go-live KEYS are still the separate owner/ops item above.)
- **Mobile performance (Lighthouse 54)** — response **compression is already in place** (`app.use(compression)`, SSE-safe via `res.flush()`), so the transfer-size quick-win is captured. Adding `defer` to `app-main.js` is NOT safe (inline scripts call its globals at parse time). Remaining lever = the **`.min.js` minify build** (esbuild/terser → minified siblings, update index.html + SW refs, keep no-store; add a Playwright smoke of the built output since the harness net tests source, not the artifact). Non-blocking; do as a deliberate babysat pass.

**Genuinely OPEN — decisions (nothing breaks; current behaviour silently becomes the decision):**
- **Real KYC/registry verification — ✅ DECIDED + Phase B BUILT 2026-09-12.** Provider = **Stripe Identity** (reuses Stripe wiring, ~$1.50/verification). Built: `accountants.kyc_status/kyc_session_id/kyc_verified_at` columns; `POST /api/accountants/kyc/start` + `GET /api/accountants/kyc/status`; identity.verification_session.* handling on the platform Stripe webhook (verified/pending/failed, idempotent); admin review shows the KYC badge. `verify-accountant-kyc` 14/0. **Remaining:** (ops) enable Stripe Identity in the Stripe dashboard (no new key); (code) accountant-facing “Start identity verification” button — ✅ DONE 2026-09-12 (profile page KYC card: badge + button → `/kyc/start` → redirect to Stripe session; reads `/kyc/status`); (code) Phase C registry-portal links — ✅ DONE 2026-09-14 (`kyc-registry.js`: ACCA/ICAEW/ICAS/CIMA/US-CPA-NASBA/CPA-Ontario/CPA-Alberta verified URLs, admin-modal wired, web-search fallback for unmapped bodies e.g. ICATT; `verify-kyc-registry` 14/0).
- **Invoicing + pricing overhaul — ✅ DONE 2026-09-12.** Final decision: **Pro invoicing is UNLIMITED**, backed by a **500/month fair-use / anti-abuse ceiling** (env-tunable `PRO_INVOICE_CEILING`; counted by created_at, un-gameable; sits after an idempotency pre-check so retries return the original row; Business/Scale truly uncapped). Copy flipped to "Unlimited invoicing & quotes"; the `invoice_limit` upsell reworded to the fair-use framing. `verify-invoice-cap` 8/0 (retuned to 500).
  - **Business price $199 → $249/mo** (stays 5 entities). Pricing card + comparison header + annual toggle updated.
  - **New "Scale" tier — $400/mo, 10 entities.** Wired end-to-end: `ENTITY_LIMITS` (server+client) scale:10; `ai-cap.js` scale tier (5,000 shared / 100 scan) + `planTier`; `/api/stripe/checkout` allowlist + env-driven priceId; plan-param passthrough. New pricing card (Start-free-trial → `startTrial('scale')`) + SCALE_FEATURES.
  - **Comparison table fixed to like-for-like + multi-entity story:** competitors compared at their multi-currency tiers, and the killer rows "5 businesses — total /mo: $249 vs $375–$450" and "10 businesses: $400 vs $750–$900". `verify-e2e-payment-flow` 11/0, `verify-rbac-enforcement` 30/0 canaries green.
  - **Marketing landing (`public/landing.html`) synced 2026-09-12:** the standalone marketing pricing page (separate file from the in-app `page-pricing`) was updated to match — Business $249, new Scale card ($400/10 entities), "Unlimited invoicing & quotes" copy, comparison header $249, and the annual toggle to 20% ("Save 20%"). NOTE remaining copy inconsistency to reconcile: landing says "30-day free trial" while the in-app comparison row says "14 days" — confirm the real trial length and align both.
  - **⚠ OWNER/OPS — REQUIRED before these prices are real in billing:** (1) update the **Stripe Business Price to $249** (or customers still get charged $199); (2) **create a Stripe Price for Scale ($400)** and set **`STRIPE_PRICE_SCALE`** in Railway env — until then Scale checkout returns "Checkout unavailable". `STRIPE_PRICE_PRO`/`STRIPE_PRICE_BUSINESS` already wired.
...
3. **Security hardening checklist — the discrete items (status per 2026-09-12 recon; verify-first).**
   ...
   *Open — before launch:*
   - [x] **MFA / 2FA — accountants: ✅ DONE 2026-09-13.** TOTP (RFC 6238, `totp.js`, no new dep); enroll/enable/disable endpoints + login gate (`mfaRequired`); secret AES-256-GCM at rest; login-page code prompt + dashboard enroll UI. `verify-accountant-mfa` 17/0 (RED-proven). **Owners' MFA — backend ✅ DONE 2026-09-13** (login gate + `/api/auth/mfa/{status,setup,enable,disable}`; secret in `users.data`, AES-GCM at rest; byte-identical login for owners who haven't enabled it; `verify-owner-mfa` 18/0, RED-proven; dormant/non-breaking until UI). **Owner MFA UI — ✅ DONE 2026-09-13**: login-page code field on the JS-rendered `showAuthGate`/`ffLogin` (`verify-owner-mfa-login-ui` 13/0) + Settings 2FA enrollment card `finflow-owner-mfa.js` with setup/enable/disable, status loaded on settings-open (`verify-owner-mfa-enroll-ui` 26/0). Feature complete end-to-end.
   ...
   - [x] **Cloudflare-aware rate-limit key — ✅ DONE 2026-09-13.** `authLimiter`/`acceptLimiter` (and the shared `_rlKey` fallback) key on the real client via `_clientIp` → `CF-Connecting-IP` first, then Envoy/XFF, then `req.ip`; correct behind Cloudflare AND Railway-only today. `verify-cloudflare-rl-key` 6/0 (RED-proven). ⚠ Spoof-proof ONLY once the Railway origin is locked to Cloudflare (Authenticated Origin Pulls or CF IP-range allowlist) — see INFRA_RUNBOOK step 7; no worse than before pre-lockdown.
   - [x] **Bcrypt cost-10 → 12 — ✅ DONE 2026-09-12** (server.js password-update path; now cost-12 everywhere).
   - [x] **Connector-token encryption at rest — ✅ HARNESS-PROVEN 2026-09-14 (`verify-connector-token-encryption` 11/0; was audit 09-13).** All 13 connectors that store a credential encTok it (AES-256-GCM, `CONNECTOR_ENC_KEY`-preferred): Plaid/Finch/Stripe access_token, WiPay api_key, WooCommerce consumer_key+secret, and the 7 OAuth-driver connectors (access+refresh token). Codat (company_id) and Belvo (link ids) store no per-user secret — server-side env key + non-secret reference. Only non-encTok token uses are in-flight Plaid API calls, not storage.
   *Open — assume-breach / detect fast:*
   ...
   - [x] **Audit-log anomaly DETECTION — ✅ DONE 2026-09-14.** `audit-anomalies.js` (read-only, env-tunable thresholds) + `GET /api/admin/audit-anomalies` (admin) detects accountant cross-client access, per-actor activity spikes, and mass delete off `audit_trail`. `verify-audit-anomalies` 10/0 (RED-proven: under-threshold + out-of-window actors stay silent). DELIVERY ✅ 2026-09-14 — email digest via Resend (`notifyAnomalies` + `POST /api/admin/audit-anomalies/notify`, recipient `SECURITY_ALERT_EMAIL`; `verify-audit-anomaly-notify` green). SIGNALS EXPANDED ✅ 2026-09-14 — login + document-download events now audited; `login_multi_ip` (distinct login IPs — honest proxy, NOT geo-velocity) + `mass_export` (document downloads) signals added; `verify-anomaly-events` green (real login writes a LOGIN row w/ ip). Plus `failed_login_burst` ✅ 2026-09-14 — failed logins (owner + accountant) audited with IP → brute-force / credential-stuffing detection (`verify-anomaly-failed-login` green). NOTE: CSV/PDF report exports are client-side (jsPDF), not server-auditable. AUTO-SCAN ✅ 2026-09-14 — `startAnomalyMonitor` runs a periodic scan→email on boot (guarded: no-op unless `SECURITY_ALERT_EMAIL` set; unref'd timer; env `AUDIT_ANOMALY_SCAN_MIN`, default 60); `verify-anomaly-monitor` green. REMAINING (optional): true geo-velocity impossible-travel (needs a geo service).
   - [~] **Error monitoring (Sentry) + uptime alerting** — UPTIME ✅ 2026-09-14: public `GET /healthz` (unauth, DB-reachability, 200 healthy / 503 degraded, no-store; `verify-healthz` green, DB-down RED-proven) — point Cloudflare/Railway/UptimeRobot at it. REMAINING: Sentry error monitoring (needs a DSN — ops).
...
1. **FX base-currency consolidation — the #1 credibility item. SERVER LAYER ✅ SHIPPED 2026-09-14 (`c005cbd`); design in `FX_CONSOLIDATION_DESIGN.md`.** `computeBooks(entityId=null)` now converts every leg per-entity to base (`users.data.base_currency` → first-entity currency → USD); `verify-fx-consolidation` 12/0, money sweep 65/65 green, single-currency byte-identical. REMAINING (client layer): the main dashboard consolidated is summed client-side (`renderConsolPL`) and still raw-sums native — point it at `?entity_id=all` (add that → entityId=null in the middleware) or fetch each entity with `?display=base` and sum. See the design doc STATUS block.** Key finding: the per-leg recognition-dated conversion engine ALREADY exists (F34 Path B `computeBooks(display)` + `fx_rates` + `fxCoverage` flag); the real gap is (a) no account base-currency setting and (b) the CONSOLIDATED (entityId=null) view defaults to native raw-sum instead of converting to base. Fix reuses the F34 engine — much smaller than 'build FX'. `computeBooks` all-entities aggregate
...
- ✅ **Time Tracking — BOTH pages** entity-scoped. **Projects** (was totally ungated: GET passed null, POST never tagged) + **Timesheet**: null-inclusive GET filter + POST entity_id tag + entity-scoped dedup + switchEntity refetches both on switch. `verify-projects-entity-scope` 9/0, `verify-timesheet-entity-scope` 7/0.
  - **DESIGN CHANGE + BACKFILL:** projects/timesheet were originally EXCLUDED from the NOT-NULL entity_id invariant (NULL = account-level, shown on every business — the "leaks into all entities" report). Per product decision they are now STRICTLY per-entity. An idempotent DB-init backfill (database.js, before the init COMMIT) homes pre-existing orphan NULL projects/timesheet rows to each user's FIRST (min-id) entity, so old rows stop appearing everywhere and nothing is lost. Runs on next deploy/boot. NOTE: orphan rows land on the user's first entity (e.g. Saige), not necessarily their true home — reassign/delete as needed. `documents`/`templates` are STILL account-level-NULL-capable (same latent leak if that's unwanted — apply the same gate+backfill).
- ✅ **(superseded) timesheet-only pass** entity-scoped (null-inclusive GET filter + POST entity_id tag + entity-scoped dedup) AND switchEntity now REFETCHES timesheet on switch (renderTimesheet paints from an in-memory cache, so server scoping alone left stale cross-entity rows on screen). `verify-timesheet-entity-scope` 7/0.
