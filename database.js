...
async function initDB() {
  ...
  try {
    ...
    // One row per unique visitor (ip+user-agent hash) per UTC day. `views` counts
    // repeat loads that day; last_seen carries the most recent hit's time. Country/
    // region/city are resolved from the IP at capture time (fast-geoip, offline).
    // Surfaced only in the admin Traffic panel. RLS enabled to stay consistent with
    // the platform lockdown (the app connects as a superuser and bypasses it).
    await client.query(`
      CREATE TABLE IF NOT EXISTS page_views (
        ...
        UNIQUE(visitor_key, view_day)
      )
    `);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_page_views_day ON page_views(view_day DESC)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_page_views_last_seen ON page_views(last_seen DESC)`);
    await client.query(`CREATE INDEX IF NOT EXISTS idx_page_views_country ON page_views(country)`);
    // Boot-safe: enabling RLS is idempotent; superuser/table-owner connections bypass it,
    // so the app is unaffected while any future Data-API exposure stays denied by default.
    await client.query(`ALTER TABLE page_views ENABLE ROW LEVEL SECURITY`).catch(() => {});
